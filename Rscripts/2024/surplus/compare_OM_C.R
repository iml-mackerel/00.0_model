...
save(Cruns,file=paste0('Rdata/',year,'/sensitivity/fitsC.Rdata'))
load(file=paste0('Rdata/',year,'/sensitivity/fitsC.Rdata'))

.wd <- paste0('img/',year,'/sensitivity/C/')
dir.create(.wd, showWarnings = FALSE)

savepng(catchplot(Cruns,ci=FALSE),.wd,"catch",c(17,10))

savepng(fitplot(Cruns,type='AIC',n=FALSE),.wd,"AIC",c(8,8))
savepng(fitplot(Cruns,type='nll',n=FALSE),.wd,"nll",c(16,8))

ssbplot0(Cruns, minyear=2010, year=year, legend=T)
ggsave(paste0(.wd,"ssb_noci2010.png"), width=6, height=5, units="in", bg="white")
ssbplot0(Cruns, minyear=1969, year=year, legend=T)
