...
problrp<- round(c(x75h,x75,x75l)/LRP,1)
...
savepng(gBI,paste0('img/',year,'/probLRP/'),"ProbLRP_BI",c(14,16))
...
savepng(gEN,paste0('img/',year,'/probLRP/'),"ProbLRP_EN",c(14,16))
...
savepng(gFR,paste0('img/',year,'/probLRP/'),"ProbLRP_FR",c(14,16))
...
sdlogssb = logssb*CV
logLRP<- log(LRP)
Llogssb = logssb -(1.96*sdlogssb) # CV max  et CV min sd <a remplacer du CV moyen - sdCV sd=  dans pnorm, meoyenne reste pareil

Ulogssb = logssb +(1.96*sdlogssb)

LlogLRP = logLRP -(1.96*sdlogssb)
UlogLRP = logLRP +(1.96*sdlogssb)


probLRP<- data.frame(logssb, CV, sdlogssb, logLRP, Llogssb, Ulogssb, LlogLRP, UlogLRP) 
...
#https://www.youtube.com/watch?v=M2v1JCjdrgQs see this very good video
probLRP <-   probLRP %>% dplyr::mutate(probLRP =pnorm(logLRP, mean=logssb, sd=sdlogssb, lower.tail=F),
                                        LprobLRP1= pnorm(logLRP, mean=Llogssb, sd=sdlogssb, lower.tail=F),
                                        UprobLRP1= pnorm(logLRP, mean=Ulogssb, sd=sdlogssb, lower.tail=F)) %>% 
                                       # LprobLRP2= pnorm(LlogLRP, mean=logssb, sd=sdlogssb, lower.tail=F),
                                        #UprobLRP2= pnorm(UlogLRP, mean=logssb, sd=sdlogssb, lower.tail=F)) %>% 
    mutate(ssb =exp(logssb),
           ratio=ssb/LRP)

probLRP %>% ggplot(aes(x=exp(logssb), y=probLRP)) +geom_point()

#%>% # lower.tail or 1-pnorm
 #    dplyr::slice(which(abs(probLRP - 0.75) == min(abs(probLRP - 0.75)) |
  #                    abs(LprobLRP2 - 0.75) == min(abs(LprobLRP2 - 0.75))|
   #                             abs(UprobLRP2 - 0.75) == min(abs(UprobLRP2 - 0.75)))) %>% 
    
        
#methode 1 et 2 donne le même résultat                                        
