...
p1a <- ggplot(sweep(cn,1,rowSums(cn),"/") %>%  as.data.frame() %>% rownames_to_column("year") %>%  
            ...
            aes(x = as.numeric(year), y = as.numeric(age), size = caa, col = year_cat, shape=shape_cat)) + 
    geom_point(alpha=0.8) + scale_size(range = c(1, 8), breaks=seq(0,0.6,0.1)) + 
...
saveplot(p1aBI,name='cn_propBI',dim=c(15,8),wd=wd,type=type)  # raw data
