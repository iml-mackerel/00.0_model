...
fits.Mct.test <- fits.Mct2[Mrange ==0.30][[1]]
...
DateDir = paste0("Rdata/",my.year,"/proj30/",Date,"/")
...
### plots ---------------------------
    ...
    save(runlist, file=paste0('Rdata/',my.year,'/proj30.Rdata'))

load(file=paste0('Rdata/',my.year,'/proj30.Rdata'))
refBase <- ypr(fit)
REF <- refBase$f40ssb
LRP <- REF*0.40
USR <- REF*0.80
...
saveplot(p_ssb,name="proj_ssb",dim=c(15,15),wd=paste0('img/',my.year,'/proj30'))
...
saveplot(p_rec,name="proj_rec",dim=c(15,15),wd=paste0('img/',my.year,'/proj30'))
...
saveplot(pIEbox,name="IEbox",dim=c(6,8),wd=paste0('img/',my.year,'/proj30'))
pIEbox <- pIEbox+labs(y='Captures (t)',x='Année')
saveplot(pIEbox,name="IEbox_FR",dim=c(6,8),wd=paste0('img/',my.year,'/proj30'))
...
saveplot(pIEboxBI,name="IEbox_BI",dim=c(10,4),wd=paste0('img/',my.year,'/proj30'))
...
saveplot(pusboxBI,name="IEboxus_BI",dim=c(6,6),wd=paste0('img/',my.year,'/proj30'))
...
names(projres)[2]=paste0('TAC', year+1)
...
write.csv(projres, file = paste0("csv/",my.year,"/proj30.csv"),row.names = FALSE)

projres=read.csv(paste0("csv/",my.year,"/proj30.csv"))
...
write.csv(projres2, file = paste0("csv/",my.year,"/proj30_mean.csv"),row.names = FALSE)
