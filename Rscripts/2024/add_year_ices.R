...
list_of_files = list.files("data/2022", full.names=T, pattern=".dat")
iny="2022"
outy="2024"
...
default<- read.csv(paste0("../02.0_catch-at-age/csv/",outy,"/caa_interpol.csv"))[1:3] %>% 
    pivot_wider(names_from=age, values_from="caan") %>%  as.data.frame()
add_rows_ices(datname="cn" , csvfile=default, nyears=year_to_remove,
              input_year=iny, output_year=outy, 
              name="NWA Mackerel catch-at-age - catch.n (units : Thousands )")
file.rename(from=paste0("data/",outy,"/cn.dat"), to=paste0("data/",outy,"/sensitivity/cn_default.dat"))


caain2224<-bind_rows(default %>%  dplyr::filter(year < 2022),
 read.csv(paste0("../02.0_catch-at-age/csv/",outy,"/caa_modified2022_2024.csv"), row.names=1)[1:3] %>% 
    pivot_wider(names_from=age, values_from="caan") %>%  as.data.frame())
add_rows_ices(datname="cn" , csvfile=caain2224, nyears=year_to_remove,
              input_year=iny, output_year=outy, 
              name="NWA Mackerel catch-at-age - catch.n (units : Thousands )")
file.rename(from=paste0("data/",outy,"/cn.dat"), to=paste0("data/",outy,"/sensitivity/modified2022_2024.dat"))


caain2223<- bind_rows(default %>%  dplyr::filter(year < 2022),
                      read.csv(paste0("../02.0_catch-at-age/csv/",outy,"/caa_modified2022_2023.csv"), row.names=1)[1:3] %>% 
    pivot_wider(names_from=age, values_from="caan") %>%  as.data.frame())
add_rows_ices(datname="cn" , csvfile=caain2223, nyears=year_to_remove,
              input_year=iny, output_year=outy, 
              name="NWA Mackerel catch-at-age - catch.n (units : Thousands )")
file.copy(from=paste0("data/",outy,"/cn.dat"), to=paste0("data/",outy,"/sensitivity/modified2022_2023.dat"), overwrite=T)



#Landings no yet calculated for 2024 # copied from 2023
#changer min et max pour 1 et 10 dans les années futures.
ctin<- read.csv(paste0("../01.0_landings/csv/",outy,"/total.csv")) %>% dplyr::select(year, total)%>%  as.data.frame() %>%  mutate(max=NA) %>%  rename(min=total)
...
## catch foreign
ctfin<- read.csv(paste0("../01.0_landings/csv/",outy,"/total.csv")) %>% dplyr::select(year, Foreign)%>%  as.data.frame() %>%  mutate(max=NA) %>%  rename(min=Foreign)
...
#stock weight
swin<- read.csv(paste0("../04.0_weight-at-age/csv/waa_",outy,"_base_cv1shrink0.csv"), header=T, check.names =F) %>% 
    as.data.frame()
...
#faa 
faa<- read.csv(paste0("../05.0_fecundity-at-age/csv/",outy,"/faa_",outy,"_base_cv1shrink0.5.csv"), header=T, check.names =F) %>% 
    as.data.frame()
...
#maa 
maa<- read.csv(paste0("../06.0_maturity-at-age/csv/",outy,"/maa",outy,"_base_smooth0.5.csv"), header=T, check.names =F) %>% 
    as.data.frame()
...
mort[,2:11] <- 0.3 #0.3 starting 2024, before was changed in 
