...
#*** Projection samples from the last 10 years with and recruitment is projected forwards using the mean with autocorrelation of 0.9
...
proj <- list(fit = fit,                                 # fitted CCAM object
               ...
               rec.years = 2011:2020,                   # years of recruitment on which projections will be based. 1968 removed
               rec.meth = 2,                            # options are 1 (Beverton Holt lag 2 autocorrelation); 2 (mean lag 1 autocorrelation); 3 (original SAM method, no autocorrelation); 4 (sample trailing years of recruitment); 5 and 6 are experimental and gave horrible results (no autocorrelation)
...
scen.list <- lapply(split(scenmat,1:nrow(scenmat)),function(x){
    ...
    MPx$IE <- c(as.character(x[1,3]),'IEdep2550')  # IEdep2550 = US missing catch scenario 
    c(OMx,MPx)
})
...
save(runlist, file = 'Rdata/2020/proj/final/proj_base_2020_mean_ac_09.Rdata')

# load(file = 'Rdata/2020/proj/proj_base_2020_BH_AC_corrected.Rdata') # folder named after system date. change dir
