...
#sens1
block1 <- matrix(c(0,1,2,3,4,4,4,4,4,4), nrow=length(c(1969:2025)), ncol=ncol(conf$keySel),byrow = T)
sens[[1]] <- rbind(block1)
...
#sens2
    ...
    block3 <- matrix(c(0,1,2,3,4,4,4,4,4,4), nrow=length(c(2024:my.year)), ncol=ncol(conf$keySel),byrow = T)
    sens[[2]] <- rbind(block1,block2, block3)
  
    
        #sens3 # does not converge
      block1 <- matrix(c(0,1,2,3,4,4,4,4,4,4), nrow=length(c(1969:1999)), ncol=ncol(conf$keySel),byrow = T)
      block2 <- matrix(c(0,1,2,3,4,4,4,4,4,4)+5, nrow=length(c(2022:my.year)), ncol=ncol(conf$keySel),byrow = T)
          
      sens[[3]] <- rbind(block1,block2)
      

    nsens=3 # scenario 8 does not always converge
...
for(i in 1:nsens){
    ...
    conf[[i]]$keyVarObs[2,1:9]=c(0,1,2,2,2,2,1,1,1) 
...
my.seltable = list()
newtable=list()
cfiles=c("default","default", "default" 
         )
