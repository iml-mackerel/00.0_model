...
if(newdir) Date <- Sys.Date() else Date <- "2025-02-12"   # continue in old directory or set new one
...
newrunlist<- runlist
names(newrunlist) <- c("OM.base - F0",paste0("OM.base TAC US=",tacus,"t"),"OM.M0.25 - F0", paste0("OM.M0.25 TAC US=",tacus,"t"),"OM.recMean - F0", paste0("OM.recMean TAC US=",tacus,"t"),
...
saveplot(tmin.ssb,name="ssb",dim=c(17,10),wd=paste0('img/',year,'/Tmin'))
...
saveplot(cz,name="cz",dim=c(17,10),wd=paste0('img/',year,'/Tmin'))

cz <- cz+ labs(y='Probabilité de sortir de la ZC',
               x='Année',color = names(runlist))+
      scale_x_continuous(breaks=year:(year+11))
saveplot(cz,name="cz_FR",dim=c(17,10),wd=paste0('img/',year,'/Tmin/'))
...
saveplot(pEN,name="cv_2panel_col",dim=c(12,14),wd=paste0('img/',year,'/Tmin') )  

pFR<- p + labs(y="Probabilité de sortir de la ZC",x='Année',col='')
saveplot(pFR,name="cv_2panel_colFR",dim=c(12,14),wd=paste0('img/',year,'/Tmin') )  
...
saveplot(pBI,name="cv_2panel_colBI",dim=c(14,14),wd=paste0('img/',year,'/Tmin') )  



p <- ggplot(df %>%  filter(IE=="F = 0"),aes(x=x,y=y*100))+
    geom_rect(xmin=-Inf,xmax=Inf,ymin=75,ymax=100,fill='lightgrey')+
    geom_point(aes(col=OM))+
    geom_line(aes(col=OM))+
    geom_text(data=rebuildf%>%  filter(IE=="F = 0"),aes(y=25,x=year+5,label=paste0("Tmin = ",V1)))+
    scale_color_viridis_d()+
    #scale_color_manual(values=c(1:5))+
    #facet_wrap(~IE,ncol=1)+
    scale_y_continuous(expand=c(0,0),limits = c(0,100))+
...
pBI<- p + labs(y="Probabilité de sortir de la ZC\nProbability (%) of getting out of the CZ",x='Année | Year',col='')+
    ...
    guides(col=guide_legend(ncol=1))
saveplot(pBI,name="cv_Tmin_colBI",dim=c(14,8),wd=paste0('img/',year,'/Tmin') )  
