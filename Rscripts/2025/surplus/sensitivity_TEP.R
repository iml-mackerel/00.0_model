...
########## plots #################################################################################################
source(paste0("Rscripts/",my.year,"/surplus/sensitivity_TEP_plot.R"))
...
sub <- inputs[c(21:200)]
multifit(sub,dir = paste0('Rdata/',my.year,'/TEP/'),parallel=FALSE)

########## output #################################################################################################
sens <- lapply(list.files(paste0('Rdata/',my.year,'/TEP/'),full.names = T),function(x) get(load(x)))
...
saveplot(p,'fits_ssblrp',paste0('img/',my.year,'/sensitivity/TEP/'),c(8,4))
...
saveplot(p,'fits_ssb',paste0('img/',my.year,'/sensitivity/TEP/'),c(12,8))
...
p <- ggplot(out[out$class=='sim' & !is.na(out$tep),],aes(x=ssb.year,y=tep,group=model))+
    ...
    labs(y='TEP',x='Année | Year') +scale_x_continuous(breaks=seq(1970, my.year, 5))

saveplot(p,'tep_new',paste0('img/',my.year,'/sensitivity/TEP/'),c(12,8))
...
saveplot(p,'tep_new_highstate',paste0('img/',my.year,'/sensitivity/TEP/'),c(12,8))

lrpr <- out[out$class=='base'& out$ssb.year==my.year,'ssbLRP']
...
p <- ggplot(low,aes(x=ssb.year,y=tep,group=model))+
    ...
    scale_y_continuous(expand=c(0,0),limits = c(0,max(out$tep,na.rm=T)))+
    labs(x='Year',y='TEP')   
saveplot(p,'tep_new_lowstate',paste0('img/',my.year,'/sensitivity/TEP/'),c(12,8))
