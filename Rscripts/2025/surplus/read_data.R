...
#save as csv for data management
dir.csv <- paste0('csv/',year,'/') 

int2<- list(data.frame(year=as.character(1969:year)), 
            survey%>%  as.data.frame() %>%  rownames_to_column("year") %>%  mutate(TEP_e12=X.1/10^9) %>%  dplyr::select(-X.1),
     ctwusa %>% as.data.frame() %>%  rownames_to_column("year")%>%  rename(Catch_lwr=min, Catch_upr=max)) %>%
purrr:: reduce(full_join) %>%  filter(year !=1968)
write.csv(int2,paste0(dir.csv, "Intrants_", year, ".csv"), row.names=F)


int1<- list(
cn %>%  as.data.frame() %>%  rownames_to_column("year") %>%  pivot_longer(2:11, names_to="age", values_to="caa_1000"),
mo %>%  as.data.frame() %>%  rownames_to_column("year") %>%  pivot_longer(2:11, names_to="age", values_to="prop_mature"),
sw %>%  as.data.frame() %>%  rownames_to_column("year") %>%  pivot_longer(2:11, names_to="age", values_to="stock_weight_kg"),
cw %>%  as.data.frame() %>%  rownames_to_column("year") %>%  pivot_longer(2:11, names_to="age", values_to="catch_weight_kg"),
fec%>%  as.data.frame() %>%  rownames_to_column("year") %>%  pivot_longer(2:11, names_to="age", values_to="fecundity")) %>% 
    purrr::reduce(full_join) %>%  filter(year !=1968)
write.csv(int1,paste0(dir.csv, "Intrants_at_age_", year, ".csv"), row.names=F)
