...
#highlight last 2 years %>%  mutate(catyear= if_else(year %in% c((my.year-1) : my.year), "1", "0"))
ssb<- ssb %>% as.matrix() %>% as.data.frame() %>%  mutate(catyear= if_else(year %in% my.year, "1", "0"))
...
p2 <- ggplot()+
    ...
    scale_y_continuous(limits=c(0,100),expand=c(0,0))+
    scale_x_continuous(expand=c(0,5), limits=c(0,400))+
...
gFR<- ggarrange(p1 + labs(x='Année',y="Probabilité d'être supérieur à PRL (%)"), 
                p2 + labs(y="Probabilité d'être supérieur à PRL (%)",x="BSR (1000' t)"), ncol=1)
