...
.wd <- paste0('img/',year,'/sensitivity/Mconsum/')
...
extend.year<- seq(max(consum$year) + 1, year)

last_row <- consum %>%
    dplyr::filter(year == max(year))

new_rows <- last_row %>%
    slice(rep(1, length(extend.year))) %>%
    mutate(year = extend.year)

consum <- bind_rows(consum, new_rows)
...
save(fitM, file=paste0('Rdata/',year,'/sensitivity/fit.Mconsum.Rdata'))
...
saveplot(resplot(fitM,fleets = 2,type=1,low=c('red','orange'),high=c('grey','green','darkgreen')),name="/res_caa_1",dim=c(10,6),wd=.wd,type=type)
saveplot(resplot(fitM,fleets = 2,type=2,out=3),name="/res_caa_2",dim=c(10,6),wd=.wd,type=type)
saveplot(resplot(fitM,fleets = 2,type=3),name="/res_caa_3",dim=c(10,6),wd=.wd,type=type)
saveplot(resplot(fitM,fleets = 2,type=4),name="/res_caa_4",dim=c(25,20),wd=.wd,type=type)
saveplot(resplot(fitM,fleets = 2,type=5,std=TRUE),name="/res_caa_5",dim=c(25,20),wd=.wd,type=type)
saveplot(resplot(fitM,fleets = 2,type=6),name="/res_caa_6",dim=c(10,6),wd=.wd,type=type)
saveplot(resplot(fitM,fleets = 2,type=7),name="/res_caa_7",dim=c(10,6),wd=.wd,type=type)
