...
MP4$catchval <- rep(540,ny)
MP5$catchval <- rep(575,ny)
MP6$catchval <- rep(600,ny)
MP7$catchval <- rep(750,ny)
MP8$catchval <- rep(1000,ny)
...
ctus3<- read.csv(paste0("data/2024/raw/USrec_decades_during_winter.csv"), dec=".")  %>%  
    dplyr::select(decade, dec_perc_rec) %>%  dplyr::distinct()
ctus3
us<- read.table(paste0("data/",my.year,"/raw/from_Kiersten.csv"), dec=".", sep=";", header=T) %>%  rename(year=X) %>%  filter(year == my.year) %>% 
...
us.ym1 = rep(us$us.ym1,3)
min.us=us.ym1 + rec# to review total of 2025

mode1 <- us.now + maxhist + disc[1] +rec[1]
mode23 <-  rec +disc + comm


tacus=comm+rec
abc=c(14634,16607,16607) #
mode= c(mode1, mode23[2:3])

pert.table<- data.frame(year=2026:2028, min.us, mode, abc)
write.csv(pert.table,"csv/2025/pert.table.csv")
...
IEindepus <- function(x,y,seed=NULL){
    ...
    min <- min.us  # based on minimun for rec fishing
    mode <- mode
    max <- abc  # abc TAC for future
...
load(file=paste0('Rdata/',my.year,'/proj.Rdata'))
refBase <- ypr(fit)
REF <- refBase$f40ssb
LRP <- REF*0.40
USR <- REF*0.80
...
saveplot(p_ssb,name="proj_ssb",dim=c(15,15),wd=paste0('img/',my.year,'/proj'))
...
saveplot(p_rec,name="proj_rec",dim=c(15,15),wd=paste0('img/',my.year,'/proj'))
...
write.csv(table_ies, file = paste0("csv/",my.year,"/proj_table_ie.csv"),row.names = FALSE)
...
saveplot(pIEhisto,name="IEdensity",dim=c(6,8),wd=paste0('img/',my.year,'/proj'))

pIEbox <- ggplot(ies,aes(x=factor(Year),y=value))+geom_boxplot()+
    facet_wrap(~name,ncol=1,scale='free_y')+labs(y='Catch (t)',x='Year')
saveplot(pIEbox,name="IEbox",dim=c(6,8),wd=paste0('img/',my.year,'/proj'))
pIEbox <- pIEbox+labs(y='Captures (t)',x='Année')
saveplot(pIEbox,name="IEbox_FR",dim=c(6,8),wd=paste0('img/',my.year,'/proj'))
...
saveplot(pIEboxBI,name="IEbox_BI",dim=c(10,4),wd=paste0('img/',my.year,'/proj'))
...
saveplot(pusboxBI,name="IEboxus_BI",dim=c(6,6),wd=paste0('img/',my.year,'/proj'))
...
write.csv(projres2, file = paste0("csv/",my.year,"/proj_mean.csv"),row.names = FALSE)
...
saveplot(pBI,name="proj_recBI",dim=c(12,6),wd=paste0('img/',my.year,'/proj') )  
