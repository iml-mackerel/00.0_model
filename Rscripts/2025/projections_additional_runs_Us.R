...
min <- rep(rec,ny)  # based on minimun for rec fishing
mode <- rep(tacus,ny) # tac for 2024-2025  US landings 2022
max <- rep(abc,ny) 
...
IEindepus <- function(x,y,seed=NULL){
    ...
    min <- min  # based on minimun for rec fishing
    mode <- mode
    max <- max  # abc TAC for future
...
### plots ---------------------------
    ...
    save(runlist, file=paste0('Rdata/',my.year,'/proj_additional.Rdata'))

load(file=paste0('Rdata/',my.year,'/proj_additional.Rdata'))
refBase <- ypr(fit)
REF <- refBase$f40ssb
LRP <- REF*0.40
USR <- REF*0.80
...
saveplot(p_ssb,name="proj_ssb_additional",dim=c(15,15),wd=paste0('img/',my.year,'/proj'))
...
saveplot(p_rec,name="proj_rec_additional",dim=c(15,15),wd=paste0('img/',my.year,'/proj'))
...
write.csv(table_ies, file = paste0("csv/",my.year,"/proj_table_ie_additional.csv"),row.names = FALSE)
...
saveplot(pIEhisto,name="IEdensity_additional",dim=c(6,8),wd=paste0('img/',my.year,'/proj'))

pIEbox <- ggplot(ies,aes(x=factor(Year),y=value))+geom_boxplot()+
    facet_wrap(~name,ncol=1,scale='free_y')+labs(y='Catch (t)',x='Year')
saveplot(pIEbox,name="IEbox_additional",dim=c(6,8),wd=paste0('img/',my.year,'/proj'))
pIEbox <- pIEbox+labs(y='Captures (t)',x='Année')
saveplot(pIEbox,name="IEbox_FR_additional",dim=c(6,8),wd=paste0('img/',my.year,'/proj'))
...
saveplot(pIEboxBI,name="IEbox_BI_additional",dim=c(10,4),wd=paste0('img/',my.year,'/proj'))
...
saveplot(pusboxBI,name="IEboxus_BI_additional",dim=c(6,6),wd=paste0('img/',my.year,'/proj'))
...
write.csv(projres, file = paste0("csv/",my.year,"/proj_additional.csv"),row.names = FALSE)

projres=read.csv(paste0("csv/",my.year,"/proj_additional.csv"))
...
write.csv(projres2, file = paste0("csv/",my.year,"/proj_mean_additional.csv"),row.names = FALSE)
...
saveplot(pBI,name="proj_recBI_additional",dim=c(12,6),wd=paste0('img/',my.year,'/proj') )  
