...
surveys[[1]][,1] <- surveys[[1]][,1]/1000
p11 <- surveyplot(surveys)+ylab('SSB (kt)')+scale_y_continuous(limits=c(0,2000),expand = c(0,0))
...
saveplot(grid.arrange(p17+ggtitle('Total Catch'),
                      ...
                      prettymatplot(surveys[[1]],ylab='SSB (kt)',xlab='Year')+ggtitle('Survey index'),ncol=3),
         name='alldat',dim=c(30,8),wd=wd,type=type)

saveplot(grid.arrange(p17+ggtitle('Total Catch'),
                      p1+theme(legend.position = 'none')+ggtitle('Catch-at-age'),
                      prettymatplot(surveys[[1]],ylab='SSB (kt)',xlab='Year')+ggtitle('Survey index'),ncol=1),
